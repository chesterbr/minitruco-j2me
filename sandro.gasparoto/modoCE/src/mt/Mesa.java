...
public class Mesa extends Canvas implements Runnable {
	...
	/**
	 * Informa a mesa o JogadorBot que está acoplado a ela
	 * (isto acontece somente no modo confronto de estratégias)
	 * 
	 * @param jogador
	 */
	...
	public void atualizaPlacarComVaquinhasInfo(int nos, int eles, int vaquinhasNoPastoDplA, int vaquinhasNoPastoDplB) {
		if (vaquinhasNoPastoDplA == 0 && vaquinhasNoPastoDplB == 0
				&& nos == 0 && eles == 0) {
			// Inicializa o placar
			limpaPlacar();
		} else {
			...
			if (placarPartidas[0] != vaquinhasNoPastoDplA) {
				animador.piscaPlacarComVaquinhasInfo(1, nos, eles, vaquinhasNoPastoDplA, vaquinhasNoPastoDplB, this.modoCE);
			} else if (placarPartidas[1] != vaquinhasNoPastoDplB) {
				animador.piscaPlacarComVaquinhasInfo(2, nos, eles, vaquinhasNoPastoDplA, vaquinhasNoPastoDplB, this.modoCE);
	...
	protected void paint(Graphics g) {
		...
		if (aberturaVisivel) {
			...
			if (aberturaUrlVisivel) {
				...
				// Para identificar a versão de testes
				g.setColor(0x990033);
				g.setFont(Font.getFont(Font.FACE_PROPORTIONAL,
					Font.STYLE_BOLD, Font.SIZE_SMALL));
				g.drawString("developer's build", getWidth() / 2,
					getHeight() - 12, Graphics.HCENTER | Graphics.BOTTOM);
