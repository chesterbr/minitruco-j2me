...
public class MiniTruco extends MIDlet implements CommandListener {
	...
	/**
	 * Estratégias escolhidas para os jogadores CPU no modo confronto de estratégias.
	 * O índice 0 corresponde à dupla B (horizontal na mesa)
	 * e o índice 1 corresponde à dupla A (vertical na mesa).
	 */
	...
	public static Command modoCECommand = new Command("Op\u00E7\u00F5es modo CE",
			Command.SCREEN, 6);
	...
	public static Command  okAvisoBTmodoCECommand = new Command("Ok", Command.OK, 1);
	...
	static final String[] OPCOES_ESTRATEGIAS = { "Sellani", "Willian", "Gasparotto v1.1", 
			"Sortear" };

	static final String[] OPCOES_ESTRATEGIAS_MODOCE = { "Sellani", "Willian", "Gasparotto v1.1",
			"Sortear" };
	
	static final String[] OPCOES_NPARTIDAS = {"1", "3", "11", "35"};
	...
	private static final String[] OPCOES_MODOCE = { "ativo"};
	...
	private static final Image[] IMAGENS_ESTRATEGIAS_MODOCE = {null, null, null, null };
	
	private static final Image[] IMAGENS_NPARTIDAS = { null, null, null, null };

	private static final Image[] IMAGENS_DEBUG = { null, null };

	private static final Image[] IMAGENS_COMPET = { null, null };
	...
	ChoiceGroup cgCompet = new ChoiceGroup("Modo confronto de estrat\u00e9gias (CE)", Choice.MULTIPLE,
			OPCOES_MODOCE, IMAGENS_COMPET);

	ChoiceGroup cgModoCEDuplaA = new ChoiceGroup(
			"Dupla A (vertical)", Choice.EXCLUSIVE,
			OPCOES_ESTRATEGIAS_MODOCE, IMAGENS_ESTRATEGIAS_MODOCE);

	ChoiceGroup cgModoCEDuplaB = new ChoiceGroup(
			"Dupla B (horizontal)", Choice.EXCLUSIVE,
			OPCOES_ESTRATEGIAS_MODOCE, IMAGENS_ESTRATEGIAS_MODOCE);
	...
	public MiniTruco() {
		...
		cgCompet.setSelectedIndex(0, conf.modoCE);
		if (cgCompet.isSelected(0)) {
			this.modoCE = true;
		} else {
			this.modoCE = false;
		}
		...
		formModoCE = new Form("Op\u00E7\u00F5es modo CE");
		...
		for (int i = 0; i < OPCOES_ESTRATEGIAS_MODOCE.length; i++) {
			cgModoCEDuplaA.setSelectedIndex(i, OPCOES_ESTRATEGIAS_MODOCE[i]
				.equals(estrategiasModoCE[1]));
			cgModoCEDuplaB.setSelectedIndex(i, OPCOES_ESTRATEGIAS_MODOCE[i]
	...
	public void commandAction(Command cmd, Displayable disp) {
		if (cmd == iniciarCommand) {
			...
			if(this.modoCE) {
				...
				jogadorBot = new JogadorBot(estrategiasModoCE[1], Display.getDisplay(this),
						(Mesa) mesa);
			...
			// Não podemos entrar em um jogo bluetooth caso o modo
			// confronto de estratégias estiver habilitado
			if(this.modoCE == true) {
				Form avisoBTmodoCE = new Form("Aviso");
				avisoBTmodoCE.append(new StringItem(null, "Jogo via bluetooth n\u00e3o pode ser acessado no modo confronto de estrat\u00e9gias"));
				avisoBTmodoCE.addCommand(okAvisoBTmodoCECommand);
				avisoBTmodoCE.setCommandListener(this);
				Display.getDisplay(this).setCurrent(avisoBTmodoCE);
			} else {
			...
			estrategiasModoCE[0] = OPCOES_ESTRATEGIAS_MODOCE[cgModoCEDuplaB.getSelectedIndex()];
			estrategiasModoCE[1] = OPCOES_ESTRATEGIAS_MODOCE[cgModoCEDuplaA.getSelectedIndex()];
			nPartidasModoCE = Integer.parseInt(OPCOES_NPARTIDAS[cgModoCEnPartidas.getSelectedIndex()]);
	...
	private void confirmaSairPartida() {
		...
		if(this.modoCE)
			 f.append(new StringItem(null, "Deseja mesmo abandonar modo confronto de estrat\u00e9gias?"));
		else f.append(new StringItem(null, "Deseja mesmo abandonar esta partida?"));
