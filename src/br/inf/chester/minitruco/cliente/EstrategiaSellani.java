...
public class EstrategiaSellani implements Estrategia 
{
	...
	/**
	 * Retorna verdadeiro ou falso a partir de um número randômico e uma porcentaem. 
	 * Esta função é utilizada quando o jogador fica na dúvida, assim o jogador fica mais dinâmico.
	 */
...
*/		
	...
	/**
	 * Retorna o índice da melhor carta para tentar fazer a mão, se não tiver uma carta boa retorna a menor.
	 */
	private int melhorCartaParaTentarFazer(SituacaoJogo s)
	{
		...
		for(int i=2;i>=0;i--)
			if(s.cartasJogador[C[i]].getValorTruco(s.manilha) > s.cartasJogadas[s.numRodadaAtual-1][maiorCartaMesa(s)].getValorTruco(s.manilha))
			{
				//será que esta é uma boa carta? se não for vou tentar outra. Só vou jogar uma manilha em cima de uma carta boa
	...
	/**
	 * Retorna a qualificação de uma carta.
	...
	/**
	 * Retorna se compensa aumentar a aposta ou não. Se eu estiver com 9, não vou pedir 6 né cabeção! 
	 */
	...
	{
//		System.out.println("\njoga() posição:" + eu(s) + " vez:" + minhaVez(s) + (minhaVez(s)==0?" mão":minhaVez(s)==3?" pé":"") + (s.posJogador==3?" Parceiro":" Adversário"));
		classificaCartas(s);
		...
		case 1:
			...
			{
			case 0:
				return C[2]; //vuo jogar a menor carta, ainda não tem muito o que fazer, né?
			case 1:
				if(qualidadeMaiorMesa(s)>=CARTA_BOA)
					return menorCartaParaMatar(s); //o cara já saiu forçando, vou tentar fazer
				else
			...
			case 3:
				...
				if(maiorCartaEDoParceiro(s))
					return C[2]; //meu parceiro é duca, vou jogar minha menor carta
		...
		case 2:
			...
			case 0:
			case 2:
				//se estou com o jogo ganho, não tenho nem o que pensar, vou deixar passar, hehehe!!!
				if(partidaGanha(s))
					return C[2]+10; //vou esconder a outra, faz a segunda ai patão!!!
				...
				if(qualidadeMinhaMaior(s)==CARTA_EXCELENTE)
				{
					...
					if(qualidadeCarta(s.cartasJogador[C[1]],s)>=CARTA_BOA)
					{
						if(s.valorProximaAposta>0 && valeAPenaAumentar(s,true) && vouOuNaoVou(80))
							return -1; //TRUCO!!!
						return C[2]; //o cara aceitou, vou passar a menorzinha só pra arrancar
					}
					return C[2]+10; //minha outra carta não é das melhores, vou esconder a pior e ir com a manilha pra última
				}
				else
				//tô sem manilha, mas vamos ver o que da pra fazer...
				if(qualidadeMinhaMaior(s)==CARTA_BOA)
				{
					//vou dar uma olhada na minha outra carta, se também for boa vou trucar 
					if(qualidadeCarta(s.cartasJogador[C[1]],s)>=CARTA_BOA)
					{
				...
				//ô mão ruim sô, não tem muito o que fazer, pelo menos já garantimos a primeira
				if(minhaVez(s)==0)
					return C[2]+(qualidadeCarta(s.cartasJogador[C[2]],s)==CARTA_RUIM?10:0);
				else
					return C[2]+(maiorCartaEDoParceiro(s)?10:!matoAdversario(s,true)?10:0);
			case 1:
				//não vou gastar uma manilha em cima de porcaria, vou deixar pro parceiro e jogar minha pior carta
			...
			case 3:
				//vamos ver se meu parceiro já garantiu essa ou se eu estou muito ruim
				if(maiorCartaEDoParceiro(s) || !matoAdversario(s,false))
					return C[2]+10; //vou até esconder
				return menorCartaParaMatar(s)+(matoAdversario(s,true)?0:10); //tô no pé, vou tentar fazer essa
			}
		break;
		...
		case 3:
			...
			//se já estou garantido vou trucar, não vai nem ter graça, hehehe!!!
			if(partidaGanha(s))
			{					
			...
			{
			case 0:
				//se eu estiver com uma carta ruim, vou jogar fechada pra fazer uma média e deixar para o parceiro 
			...
			case 1:
				...
				return C[0];
			case 2:
				//eu não melhoro a do parceiro, mas acho que vou trucar com a carta dele
			...
			case 3:
				...
				if(matoAdversario(s,false) || maiorCartaEDoParceiro(s) || (primeiraENossa(s) && matoAdversario(s,true)))
				{
					...
					return C[0]+(matoAdversario(s,true)?0:10); //aeheh, o mané aceitou!!!
				}				
	...
	{
//		System.out.println("\naceitaTruco() vez:" + minhaVez(s) + " posição:" + eu(s) + (eu(s)==0?" mão":eu(s)==3?" pé":"") + (s.posJogador==3?" Parceiro":" Adversário"));
		classificaCartas(s);
		...
		case 2:
			...
			if((minhaVez(s)==0 && vezTrucador(s)==3) ||	(minhaVez(s)==2 && vezTrucador(s)==3))
			{
				//o pé ainda tem que fazer duas, e eu ainda tenho uma boa carta na mesa?!? Será que tá blefando?
