...
public class EstrategiaGasparotto implements Estrategia {
	 ...
	 * aleatoriedade ao jogo Para não ficar evidente as estratégias... Confundir
	 * os adversários... E dar um toque humano à CPU...
	 */
	...
	private int menorCartaParaMatarOuAmarrar(SituacaoJogo s) {
		// se eu não puder matar ou amarrar a carta do adversário retorno a
		// menor
	...
	private boolean partidaGanhaParaAdversario(SituacaoJogo s) {
		...
		if (s.numRodadaAtual == 3 && !maiorCartaENossa(s)) {
			...
			// sairam também, mas isso seria (quase que) impossível...
		}
	...
	private int qualidadeCarta(Carta carta, SituacaoJogo s) {
		...
		qcarta = LIXO; // apenas para ter a variável
		...
		// Obs: logicamente isso não se aplica para o caso de
		// manilha velha...

		// Workaround devido ao fato da propriedade 'manilha'
		// não poder ser utilizada diretamente
		// Estas são cartas fictícias somente para os testes
	 ...
	 * sorte. E considerando que se estiver com 9, não vou pedir 6 (e idéias
	...
	private int joga_primeira_rodada_posmao(SituacaoJogo s) {
		...
		// Senão tentar matar com a menor manilha (menos o zap) ou um
		// três pelo menos
		...
		if (qualidadeMaiorMesa(s) == TRES) {
			...
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ZAP
				...
				// Jogar o três
				return C[1];
			// Vai que temos uma manilha também (C[1]) (ou
			// mesmo outro três),
			// mesmo assim vamos jogar o três
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ZAP
					&& qualidadeCarta(s.cartasJogador[C[2]], s) == TRES)
		...
		if (qualidadeMaiorMesa(s) == AS) {
			...
			if (qualidadeCarta(s.cartasJogador[C[2]], s) == ESPADILHA)
				return C[0]; // hummmm... podemos jogar o zap
			// para tentar enganar os adversários...
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ESCOPETA)
				return C[0];
			if (qualidadeCarta(s.cartasJogador[C[1]], s) == ESCOPETA)
				return C[0]; // hummmm... podemos jogar o zap
	...
	private int joga_primeira_rodada_parape(SituacaoJogo s) {
		...
		// Se for um dois, vamos reforçar só se tivermos manilha (menos
		// o zap) ou a menor, caso contrário
		// Se for um ás, vamos reforçar, se temos zap e três, jogar o
		// três, jogar manilha (menos o zap), três/dois ou a menor, caso
		// contrário
		// Se for lixo (menor que ás), vamos reforçar, se temos zap e
		// três, jogar o três, jogar manilha (menos o zap) ou um
		// três/dois/ás/zap (estamos secos?) ou a menor, caso contrário
		...
		if (qualidadeMaiorMesa(s) <= AS) {
			...
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ZAP)
				return C[0];
			// Aqui jogamos a melhor que temos então desde que não
			// seja menor que a que está já na mesa
	...
	private int joga_segunda_rodada_mao(SituacaoJogo s) {
		...
		// Se estiver com duas cartas acima ou igual a um ás, jogar a
		// menor.
		// Se estiver com uma carta acima ou igual a um ás e um lixo,
		...
		if (partidaGanha(s)) {
			...
			if (qualidadeCarta(s.cartasJogador[C[1]], s) >= AS)
				return C[1] + 10;
			// para não ficar muito evidente toda hora a mesma
			// jogada:
			if (mandaBala(50))
				return C[1] + 10;
			return C[1];
	...
	private int joga_segunda_rodada_parape(SituacaoJogo s) {
		...
		// Se estiver com uma carta acima ou igual a um ás, jogar
		// aumentando a aposta (com alta probabilidade).
	...
	private int joga_segunda_rodada_pe(SituacaoJogo s) {
		...
		if (maiorCartaEDoParceiro(s))
			return C[1] + 10; // é isso aí parceiro! Retornar encoberto, por
		// quê revelar?...
	...
	private int joga_terceira_rodada_parape(SituacaoJogo s) {
		...
		// Se estiver com a partida ganha, arregaçar os caras!
		// Se a maior for do parceiro, e for manilha ou três, deixar
		// passar (c/ alta prob.) (já que ele não trucou não vou trucar também)
		// Se não puder com a que estiver vindo, jogar aumentando a
		// aposta no facão (com baixa probabilidade).
		// Se puder com a que estiver vindo, e estiver com manilha,
		// jogar aumentando a aposta (com média-alta probabilidade).
	...
	private int joga_terceira_rodada_pe(SituacaoJogo s) {
		...
		// Se estiver com a partida ganha, arregaçar os caras!
		// Se não puder com a que estiver vindo, jogar aumentando a
		// aposta no facão (com média probabilidade).
	...
	public boolean aceitaTruco(SituacaoJogo s) {
		...
		case 1:
			...
			// se eu tiver uma manilha e na mesa já estiver pelo
			// menos um 3 nosso, aceito
			if (qualidadeCarta(s.cartasJogador[C[0]], s) >= TRES
					&& maiorCartaENossa(s) && qualidadeMaiorMesa(s) >= TRES)
				return true;
			// se eu tiver uma três e na mesa já estiver pelo menos
			// um 3 nosso, aceito
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == TRES
					&& maiorCartaENossa(s) && qualidadeMaiorMesa(s) >= TRES)
				return true;
			// se eu tiver manilha e três pelo menos, aceito sem
			// pensar
			if (qualidadeCarta(s.cartasJogador[C[0]], s) > TRES
					&& qualidadeCarta(s.cartasJogador[C[1]], s) >= TRES)
				return true;
		...
		case 2:
			...
			if (primeiraENossa(s)) {
				// se na mesa já estiver pelo menos um 3 nosso,
				// aceito
				...
				// se a primeira NÃO for nossa (pode tbm estar
				// amarrado):
				// se eu tiver uma manilha e na mesa já estiver
		...
		case 3:
			...
			// se eu já joguei, e meu parceiro ainda não, vou deixar
			// para ele decidir
			...
			// se a primeira for nossa:
			if (primeiraENossa(s)) {
				// se na mesa já estiver uma manilha nossa,
				// aceito com alta prob.
				if (maiorCartaENossa(s) && qualidadeMaiorMesa(s) > TRES
						&& mandaBala(80))
					return true;
				// se na mesa já estiver um 3 nosso, aceito com
				// média-alta prob.
				if (maiorCartaENossa(s) && qualidadeMaiorMesa(s) == TRES
						&& mandaBala(50))
					return true;
				// se na mesa já estiver um 2 nosso, aceito com
				// média-baixa prob.
				...
				if (minhaVez(s) > vezTrucador(s)) {
					...
					if (qualidadeCarta(s.cartasJogador[C[0]], s) > TRES
							&& mandaBala(80))
						return true;
					// se eu tiver um três, aceito com
					// média-alta prob.
					if (qualidadeCarta(s.cartasJogador[C[0]], s) == TRES
							&& mandaBala(50))
						return true;
